...
ссылки/телефоны. Автоматический `needs_human`/handoff пока отключён: если модель
не знает факт, она не должна его придумывать; для записи спрашивает, когда удобно
клиенту, и не придумывает свободные окна преподавателя. Открытые задачи по
устойчивому message-id/fingerprint и формату передачи диалога — в `BACKLOG.md`.
