...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            (OUT / "list_aria.yaml").write_text(page.locator("body").aria_snapshot(), encoding="utf-8")
            print("снапшот списка -> logs/chats/list_aria.yaml")

            if order_id:
                page.goto(f"https://profi.ru/backoffice/r.php?id={order_id}&filter=open",
                ...
                for inp in page.locator("input, textarea").all():
                    try:
                        print(f"<{inp.evaluate('e => e.tagName')}> ph={inp.evaluate('e => e.placeholder')!r} "
                ...
                for b in page.locator("button").all()[:15]:
                    try:
                        ...
                        pass
                (OUT / f"dialog_{order_id}.yaml").write_text(page.locator("body").aria_snapshot(), encoding="utf-8")
