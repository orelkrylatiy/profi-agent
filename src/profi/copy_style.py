...
CHAT_STYLE_OVERRIDE = (
    "ФИНАЛЬНЫЙ СТИЛЬ ЧАТА (эти правила важнее общих инструкций выше): "
    "сначала ответь на последний прямой вопрос клиента. Потом максимум одна "
    "короткая фраза про следующий шаг. Не повторяй самопрезентацию и методику "
    "из первого сообщения. Обычно хватает 1–3 предложений и одного вопроса. "
    "Если клиент спросил цену, формат или время, дай известный факт в первой "
    "фразе без вступления. НЕ ПРИДУМЫВАЙ свободные окна, даты или конкретное "
    "время: у тебя нет календаря преподавателя. Если нужно перейти к записи, "
    "просто спроси, когда клиенту удобно. needs_human временно отключён: в JSON "
    "всегда ставь needs_human=false и отвечай сам по известным фактам. Если "
    "какого-то условия не знаешь, не выдумывай и не обещай — скажи нейтрально, "
    "что этот момент нужно уточнить, либо задай один уточняющий вопрос. "
)
...
def style_retry_instruction(issues: Sequence[str]) -> str:
    """Outreach-only feedback appended after a draft failed the style check."""
...
def chat_retry_instruction(issues: Sequence[str]) -> str:
    """Chat-specific retry: no outreach experiment language or invented schedule."""
    short = "; ".join(str(issue) for issue in issues[:5])
    return (
        " Предыдущий ответ в чате отклонён только по стилю: "
        + short
        + ". Перепиши только ответ на последнее сообщение клиента. Сохрани факты, "
        "ничего нового не придумывай, 1–3 коротких предложения, максимум один вопрос. "
        "Не предлагай конкретные свободные даты или время от себя; если нужно "
        "согласовать занятие, спроси, когда клиенту удобно. needs_human=false."
    )
