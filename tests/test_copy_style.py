...
class TestClientCopyIssues:
    def test_compact_direct_message_passes(self):
        text = (
            "Здравствуйте! Работаю только онлайн. Если формат подходит, можно начать "
            "с пробного занятия и после него решить, на что сделать упор. Когда вам удобнее?"
        )
    ...
    def test_photo_like_generated_scaffold_is_flagged(self):
        text = (
            ...
            "Поэтому на занятиях много говорим и слушаем живую речь, разбираем рабочие фразы. "
            "Формат дистанционный, урок 60 или 90 минут. "
...
class TestClientCopyMiddleware:
    def test_outreach_style_retry_returns_better_second_draft(self, monkeypatch):
        ...
        first = _raw(
            ...
            reason="подходит",
            text=(
                "Здравствуйте! Задача понятна. Поэтому на занятиях разберём всё по шагам. "
                "Формат дистанционный. Когда удобно провести пробное? И в какое время заниматься?"
            ),
        )
        second = _raw(
            verdict="send",
            reason="подходит",
            text=(
                "Здравствуйте! Могу помочь с подготовкой. На первом занятии посмотрим, "
                "что уже получается, и решим, с чего начать. Когда вам удобно попробовать?"
            ),
        )
    ...
    def test_retry_failure_keeps_first_valid_response(self, monkeypatch):
        first = _raw(
            ...
            reason="подходит",
            text=(
                "Здравствуйте! Задача понятна. Поэтому на занятиях сначала разберём базу. "
                "Когда удобно провести пробное занятие?"
            ),
        )
        calls = 0
