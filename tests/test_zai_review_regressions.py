...
def _configure_pay_fastpath(monkeypatch):
...
def test_hidden_marker_retries_after_first_body_read_failure(monkeypatch):
    monkeypatch.setattr(respond_mod.time, "sleep", lambda _: None)
    page = _PageWithBody([RuntimeError("body not ready"), "Клиент отменил заказ"])

    assert respond_mod.hidden_marker(page, retry=True) == "клиент отменил заказ"


def test_hidden_marker_does_not_match_generic_client_description(monkeypatch):
    monkeypatch.setattr(respond_mod.time, "sleep", lambda _: None)
    page = _PageWithBody(
        [
...
def test_ambiguous_missing_respond_ui_is_technical_failed(monkeypatch):
    ...
    _configure_pay_fastpath(monkeypatch)
    store = _Store()
    monkeypatch.setattr(
...
def test_llm_cooldown_fallback_reaches_real_send_path(monkeypatch):
    ...
    _configure_pay_fastpath(monkeypatch)
    store = _Store()
    fallback = "Здравствуйте! Могу помочь с задачей. Предлагаю начать с пробного занятия, определить текущий уровень и дальше составить понятный план работы. Когда вам удобно?"
...
def test_commission_pause_uses_business_date(tmp_path: Path, monkeypatch):
    import profi.fastpath as fastpath
